...
const WP_API_BASE =
  import.meta.env.WP_API_BASE || "https://technocracy.indonesiabrain.com/wp-json/wp/v2/posts";
...
export async function fetchArticleForHighlight(item: Highlight): Promise<ArticleData | null> {
  ...
  for (const url of tryUrls) {
    ...
    if (exact) {
      ...
      if (!mapped.link) {
        const wpBase = import.meta.env.WP_BASE_URL || "technocracy.indonesiabrain.com";
